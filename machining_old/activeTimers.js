...
async function loadTasksForMachine(machineId) {
    ...
    try {
        ...
        const data = await fetchTasks({ machineId, inPlan: true, completionDateIsNull: true, ordering: 'plan_order' }, 'machining');
    ...
    const placeholderTask = {
        ...
        onClick: async () => {
            ...
            try {
                ...
                const codes = await fetchHoldTasks('machining');
            ...
            document.getElementById('select-reason-code-btn').onclick = () => {
                ...
                window.location.href = `/machining/tasks/?machine_id=${encodeURIComponent(machineId)}&key=${encodeURIComponent(reasonCode)}&name=${encodeURIComponent(reasonName)}&hold=1`;
            };
        }
    };
